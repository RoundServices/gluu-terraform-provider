...
func TestAccGluuDataSourceOpenidClient_basic(t *testing.T) {
	t.Parallel()
	clientId := acctest.RandomWithPrefix("tf-acc-test")
	dataSourceName := "data.gluu_openid_client.test"
	resourceName := "gluu_openid_client.test"

	resource.Test(t, resource.TestCase{
		Steps: []resource.TestStep{
			{
				Config: testAccGluuOpenidClientConfig(clientId),
...
func TestAccGluuDataSourceOpenidClient_extraConfig(t *testing.T) {
	t.Parallel()
	clientId := acctest.RandomWithPrefix("tf-acc-test-extra-config")
	dataSourceName := "data.gluu_openid_client.test_extra_config"
	resourceName := "gluu_openid_client.test_extra_config"

	resource.Test(t, resource.TestCase{
		Steps: []resource.TestStep{
			{
				Config: testAccGluuOpenidClientConfig_extraConfig(clientId),
				Check: resource.ComposeAggregateTestCheckFunc(
...
func testAccGluuOpenidClientConfig(clientId string) string {
	return fmt.Sprintf(`
...
data "gluu_openid_client" "test" {
	client_id = gluu_openid_client.test.client_id

	depends_on = [
		gluu_openid_client.test,
	]
}
`, clientId, clientId)
}

func testAccGluuOpenidClientConfig_extraConfig(clientId string) string {
	return fmt.Sprintf(`
resource "gluu_openid_client" "test_extra_config" {
	name                     = "%s"
	client_id                = "%s"
...
data "gluu_openid_client" "test_extra_config" {
	client_id = gluu_openid_client.test_extra_config.client_id

	depends_on = [
		gluu_openid_client.test_extra_config,
	]
}
`, clientId, clientId)
}
